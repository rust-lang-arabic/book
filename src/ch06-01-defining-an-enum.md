...
مناسبة لأن قيمة التعداد يمكن أن تكون فقط واحدة من متغيراتها (variants).
...
بدلاً من تعداد داخل هيكل، يمكننا وضع البيانات مباشرة في كل متغير (variant)
من التعداد. هذا التعريف الجديد لتعداد `IpAddr` يقول إن كل من متغيري (variants) `V4` و `V6`
...
التعدادات. هنا دالة تسمى `call` يمكننا تعريفها على تعداد (enum) `Message` الخاص بنا:
...
متغير التعداد (enum) الموجود لديه، ويمكن لهذا الكود استخدام البيانات الموجودة داخل القيمة
المطابقة.
