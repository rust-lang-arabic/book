...
حتى الآن، جميع الأمثلة في هذا الفصل عرّفت وحدات (modules) متعددة في ملف واحد. عندما تصبح الوحدات كبيرة، قد ترغب في نقل تعريفاتها إلى ملف منفصل لجعل الكود أسهل في التنقل.

على سبيل المثال، لنبدأ من الكود في القائمة 7-17 الذي كان يحتوي على وحدات (modules) مطعم متعددة. سنستخرج الوحدات إلى ملفات بدلاً من تعريف جميع الوحدات في ملف جذر الصندوق (crate root). في هذه الحالة، ملف جذر الصندوق هو _src/lib.rs_، لكن هذا الإجراء يعمل أيضاً مع الصناديق الثنائية (binary crates) التي يكون ملف جذرها _src/main.rs_.

أولاً، سنستخرج وحدة (module) `front_of_house` إلى ملفها الخاص. احذف الكود داخل الأقواس المعقوفة لوحدة `front_of_house`، مع ترك تصريح `mod front_of_house;` فقط، بحيث يحتوي _src/lib.rs_ على الكود الموضح في القائمة 7-21. لاحظ أن هذا لن يُترجم حتى ننشئ ملف _src/front_of_house.rs_ في القائمة 7-22.
...
إذا وضعنا بدلاً من ذلك _hosting.rs_ في دليل _src_، سيتوقع المصرِّف أن كود _hosting.rs_ موجود في وحدة (module) `hosting` مُصرَّح بها في جذر الصندوق (crate root) وليس مُصرَّح بها كوحدة فرعية من وحدة `front_of_house`. قواعد المصرِّف حول أي ملفات يجب التحقق منها لكود أي وحدات (modules) تعني أن الأدلة والملفات تطابق شجرة الوحدات (module tree) بشكل أوثق.

> ### مسارات ملفات بديلة
>
> حتى الآن قمنا بتغطية مسارات الملفات الأكثر شيوعاً التي يستخدمها مصرِّف Rust، لكن Rust يدعم أيضاً أسلوباً قديماً لمسار الملف. بالنسبة لوحدة (module) تُسمى `front_of_house` مُصرَّح بها في جذر الصندوق (crate root)، سيبحث المصرِّف عن كود الوحدة في:
>
...
> - _src/front_of_house/hosting/mod.rs_
>
> إذا استخدمت كلا الأسلوبين لنفس الوحدة، ستحصل على خطأ في المصرِّف. استخدام مزيج من كلا الأسلوبين لوحدات (modules) مختلفة في نفس المشروع مسموح به لكنه قد يكون مربكاً للأشخاص الذين يتنقلون في مشروعك.
>
...
لاحظ أن جملة `pub use crate::front_of_house::hosting` في _src/lib.rs_ لم تتغير أيضاً، ولا يكون لـ `use` أي تأثير على أي ملفات يتم ترجمتها كجزء من الصندوق (crate). الكلمة المفتاحية `mod` تُصرِّح الوحدات (modules)، وRust يبحث في ملف بنفس اسم الوحدة (module) عن الكود الذي يدخل في تلك الوحدة.

## ملخص

Rust يتيح لك تقسيم حزمة (package) إلى صناديق (crates) متعددة وصندوق (crate) إلى وحدات (modules) بحيث يمكنك الإشارة إلى عناصر معرّفة في وحدة (module) من وحدة أخرى. يمكنك القيام بذلك عن طريق تحديد مسارات (paths) مطلقة أو نسبية. يمكن إحضار هذه المسارات إلى النطاق (scope) باستخدام جملة `use` بحيث يمكنك استخدام مسار (path) أقصر لاستخدامات متعددة للعنصر في ذلك النطاق. كود الوحدة خاص (private) بشكل افتراضي، لكن يمكنك جعل التعريفات عامة (public) بإضافة الكلمة المفتاحية `pub`.
