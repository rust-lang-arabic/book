...
هنا، نعلن عن سمة (trait) باستخدام الكلمة المفتاحية `trait` ثم اسم السمة، وهو `Summary` في هذه الحالة. نعلن أيضًا السمة كـ `pub` حتى تتمكن الصناديق التي تعتمد على هذا الصندوق من استخدام هذه السمة أيضًا، كما سنرى في بعض الأمثلة. داخل الأقواس المعقوفة، نعلن عن توقيعات الدوال التي تصف سلوكيات الأنواع التي تنفذ (implementation) هذه السمة، والتي في هذه الحالة هي `fn summarize(&self) -> String`.

بعد توقيع الدالة، بدلاً من توفير تنفيذ (implementation) داخل الأقواس المعقوفة، نستخدم فاصلة منقوطة. يجب على كل نوع ينفذ هذه السمة (trait) أن يوفر سلوكه المخصص لجسم الدالة. سيفرض المصرِّف أن أي نوع له سمة `Summary` سيكون لديه الدالة `summarize` معرفة بهذا التوقيع بالضبط.
...
تنفيذ (implementation) سمة (trait) على نوع مشابه لتنفيذ الدوال العادية. الفرق هو أنه بعد `impl`، نضع اسم السمة التي نريد تنفيذها، ثم نستخدم الكلمة المفتاحية `for`، ثم نحدد اسم النوع الذي نريد تنفيذ السمة له. داخل كتلة `impl`، نضع توقيعات الدوال التي عرّفها تعريف السمة. بدلاً من إضافة فاصلة منقوطة بعد كل توقيع، نستخدم الأقواس المعقوفة ونملأ جسم الدالة بالسلوك المحدد الذي نريده لدوال السمة لهذا النوع المعين.

الآن بعد أن نفذت (implementation) المكتبة سمة (trait) `Summary` على `NewsArticle` و `SocialPost`، يمكن لمستخدمي الصندوق استدعاء دوال السمة على نسخ من `NewsArticle` و `SocialPost` بنفس الطريقة التي نستدعي بها الدوال العادية. الفرق الوحيد هو أن المستخدم يجب أن يجلب السمة إلى النطاق بالإضافة إلى الأنواع. إليك مثالاً على كيفية استخدام صندوق ثنائي لمكتبة `aggregator` الخاصة بنا:
...
في بعض الأحيان يكون من المفيد وجود سلوك افتراضي لبعض أو كل الدوال في سمة (trait) بدلاً من طلب تنفيذات (implementations) لجميع الدوال على كل نوع. ثم، عندما ننفذ (implementation) السمة على نوع معين، يمكننا الاحتفاظ بالسلوك الافتراضي لكل دالة أو تجاوزه.
...
على الرغم من أننا لم نعد نعرّف دالة `summarize` على `NewsArticle` مباشرة، فقد قدمنا تنفيذًا (implementation) افتراضيًا وحددنا أن `NewsArticle` ينفذ سمة (trait) `Summary`. ونتيجة لذلك، لا يزال بإمكاننا استدعاء دالة `summarize` على نسخة من `NewsArticle`، مثل هذا:
...
إنشاء تنفيذ (implementation) افتراضي لا يتطلب منا تغيير أي شيء حول تنفيذ `Summary` على `SocialPost` في القائمة 10-13. السبب هو أن صيغة تجاوز تنفيذ افتراضي هي نفس صيغة تنفيذ دالة سمة (trait) لا تحتوي على تنفيذ افتراضي.

يمكن للتنفيذات (implementations) الافتراضية استدعاء دوال أخرى في نفس السمة (trait)، حتى لو لم يكن لتلك الدوال الأخرى تنفيذ (implementation) افتراضي. بهذه الطريقة، يمكن للسمة توفير الكثير من الوظائف المفيدة وتتطلب فقط من المنفذين تحديد جزء صغير منها. على سبيل المثال، يمكننا تعريف سمة `Summary` لتحتوي على دالة مرتبطة (associated function) `summarize_author` التي يكون تنفيذها مطلوبًا، ثم تعريف دالة `summarize` التي لها تنفيذ افتراضي يستدعي دالة `summarize_author`:
...
بعد تعريف `summarize_author`، يمكننا استدعاء `summarize` على نسخ من بنية `SocialPost`، وسيستدعي التنفيذ (implementation) الافتراضي لـ `summarize` تعريف `summarize_author` الذي قدمناه. نظرًا لأننا نفذنا `summarize_author`، فقد أعطتنا سمة (trait) `Summary` سلوك دالة `summarize` دون مطالبتنا بكتابة المزيد من الكود. إليك كيف يبدو:
...
باستخدام حد سمة (trait bound) مع كتلة `impl` التي تستخدم معاملات الأنواع العمومية (generic type parameters)، يمكننا تنفيذ (implementation) الدوال بشكل مشروط للأنواع التي تنفذ السمات (traits) المحددة. على سبيل المثال، النوع `Pair<T>` في القائمة 10-15 ينفذ دائمًا الدالة المرتبطة (associated function) `new` لإرجاع نسخة جديدة من `Pair<T>` (تذكر من قسم ["صيغة الدوال"][methods]<!-- ignore --> من الفصل 5 أن `Self` هو اسم مستعار لنوع كتلة `impl`، والذي في هذه الحالة هو `Pair<T>`). لكن في كتلة `impl` التالية، `Pair<T>` ينفذ دالة `cmp_display` فقط إذا كان نوعه الداخلي `T` ينفذ سمة (trait) `PartialOrd` التي تمكن المقارنة _و_ سمة `Display` التي تمكن الطباعة.
...
يمكننا أيضًا تنفيذ (implementation) سمة (trait) بشكل مشروط لأي نوع ينفذ سمة أخرى. تنفيذات (implementations) سمة على أي نوع يفي بحدود السمات (trait bounds) تسمى _التنفيذات الشاملة_ (blanket implementations) وتُستخدم على نطاق واسع في مكتبة Rust القياسية. على سبيل المثال، تنفذ المكتبة القياسية سمة `ToString` على أي نوع ينفذ سمة `Display`. كتلة `impl` في المكتبة القياسية تبدو مشابهة لهذا الكود:
...
لأن المكتبة القياسية لديها هذا التنفيذ (implementation) الشامل، يمكننا استدعاء دالة `to_string` المعرفة بواسطة سمة (trait) `ToString` على أي نوع ينفذ سمة `Display`. على سبيل المثال، يمكننا تحويل الأعداد الصحيحة إلى قيم `String` المقابلة لها مثل هذا لأن الأعداد الصحيحة تنفذ `Display`:
