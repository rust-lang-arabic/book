...
تأتي الأنماط في شكلين: قابلة للدحض (refutable) وغير قابلة للدحض (irrefutable). الأنماط التي ستطابق أي قيمة ممكنة يتم تمريرها هي _غير قابلة للدحض_. مثال على ذلك هو `x` في التعليمة `let x = 5;` لأن `x` تطابق أي شيء وبالتالي لا يمكن أن تفشل في المطابقة. الأنماط التي يمكن أن تفشل في المطابقة لبعض القيم الممكنة هي _قابلة للدحض_. مثال على ذلك سيكون `Some(x)` في التعبير `if let Some(x) = a_value` لأنه إذا كانت القيمة في المتغير `a_value` هي `None` بدلاً من `Some`، فإن النمط `Some(x)` لن يطابق.
