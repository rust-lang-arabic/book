...
هناك سبب لكون واجهات برمجة التطبيقات لإنشاء الخيوط (threads) وإنشاء المهام (tasks) متشابهة جدًا. الخيوط تعمل كحدود لمجموعات من العمليات المتزامنة (concurrent)؛ التزامن (concurrency) ممكن _بين_ الخيوط. المهام تعمل كحدود لمجموعات من العمليات _غير المتزامنة_ (asynchronous)؛ التزامن ممكن _بين_ و _داخل_ المهام، لأن المهمة (task) يمكنها التبديل بين المستقبلات (futures) في نصها. أخيرًا، المستقبلات هي أصغر وحدة تزامن في Rust، وكل مستقبل (future) قد يمثل شجرة من المستقبلات الأخرى. بيئة التشغيل (runtime) - على وجه الخصوص، منفذها (executor) - تدير المهام، والمهام تدير المستقبلات. من هذا المنظور، المهام تشبه الخيوط الخفيفة المدارة من بيئة التشغيل مع قدرات إضافية تأتي من كونها مدارة من بيئة تشغيل بدلاً من نظام التشغيل.
