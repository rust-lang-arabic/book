...
عدد المراجع (reference count) لنسخ `Rc<List>` في كل من `a` و `b` هو 2 بعد أن نغير القائمة في `a` لتشير إلى `b`. في نهاية `main`، تسقط (drops) Rust المتغير `b`، مما يقلل عدد المراجع لنسخة `b` `Rc<List>` من 2 إلى 1. لن يتم إسقاط (drop) الذاكرة التي لدى `Rc<List>` على الكومة في هذه النقطة لأن عدد مراجعها هو 1، وليس 0. بعد ذلك، تسقط Rust `a`، مما يقلل عدد المراجع لنسخة `a` `Rc<List>` من 2 إلى 1 أيضًا. لا يمكن إسقاط ذاكرة هذه النسخة أيضًا، لأن نسخة `Rc<List>` الأخرى لا تزال تشير إليها. ستبقى الذاكرة المخصصة للقائمة غير مجموعة إلى الأبد. لتصور هذه الدورة المرجعية (reference cycle)، أنشأنا المخطط في الشكل 15-4.
...
بالمقارنة مع برنامج في العالم الحقيقي، عواقب إنشاء دورة مرجعية (reference cycle) في هذا المثال ليست خطيرة جدًا: مباشرة بعد إنشاء الدورة المرجعية، ينتهي البرنامج. ومع ذلك، إذا خصص برنامج أكثر تعقيدًا الكثير من الذاكرة في دورة (cycle) واحتفظ بها لفترة طويلة، سيستخدم البرنامج ذاكرة أكثر مما يحتاج وقد يطغى على النظام، مما يتسبب في نفاد الذاكرة المتاحة.
...
عندما ينتهي النطاق الداخلي، تخرج `branch` عن النطاق وينخفض العدد القوي لـ `Rc<Node>` إلى 0، لذلك يتم إسقاط (drop) `Node` الخاص بها. العدد الضعيف 1 من `leaf.parent` ليس له تأثير على ما إذا كان سيتم إسقاط `Node` أم لا، لذلك لا نحصل على أي تسربات ذاكرة (memory leaks)!
