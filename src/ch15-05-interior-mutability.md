...
عند إنشاء مراجع غير قابلة للتغيير وقابلة للتغيير، نستخدم صيغة `&` و `&mut`، على التوالي. مع `RefCell<T>`، نستخدم وسيلتي `borrow` و `borrow_mut`، اللتان تنتميان إلى الواجهة البرمجية الآمنة التي تنتمي إلى `RefCell<T>`. تعيد وسيلة `borrow` نوع المؤشر الذكي (smart pointer) `Ref<T>`، وتعيد `borrow_mut` نوع المؤشر الذكي `RefMut<T>`. كلا النوعين ينفذان `Deref`، لذلك يمكننا معاملتهما كمراجع عادية.
