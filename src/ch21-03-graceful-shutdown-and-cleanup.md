...
يخبرنا الخطأ أننا لا يمكننا استدعاء `join` لأننا فقط لدينا استعارة (borrow) قابلة للتعديل (mutable) من كل `worker` و`join` يأخذ ملكية (ownership) وسيطته (argument). لحلّ هذه المشكلة، نحتاج إلى نقل الخيط (thread) خارج نسخة (instance) `Worker` التي تمتلك `thread` بحيث يتمكّن `join` من استهلاك الخيط. إحدى الطرق للقيام بذلك هي أخذ نفس النهج الذي اتخذناه في القائمة (Listing) 18-15. إذا كان `Worker` يحمل `Option<thread::JoinHandle<()>>`، يمكننا استدعاء طريقة (method) `take` على `Option` لنقل القيمة خارج المتغيّر (variant) `Some` وترك متغيّر `None` في مكانه. بعبارة أخرى، `Worker` الذي يعمل سيكون لديه متغيّر `Some` في `thread`، وعندما أردنا تنظيف `Worker`، سنستبدل `Some` بـ `None` بحيث لن يكون لدى `Worker` خيط ليشغّله.
...
في هذه الحالة، يوجد بديل أفضل: طريقة (method) `Vec::drain`. تقبل معامل (parameter) نطاق (range) لتحديد أي عناصر لإزالتها من المتجه (vector) وتُرجع مُكرِّرًا (iterator) من تلك العناصر. سيؤدي تمرير بناء `..` range إلى إزالة _كل_ قيمة من المتجه.
...
يحلّ هذا خطأ المصرِّف (compiler) ولا يتطلّب أي تغييرات أخرى على كودنا (code). لاحظ أنه، لأن يمكن استدعاء drop عند الذعر (panicking)، يمكن أن يصاب unwrap أيضًا بالذعر (panic) ويتسبّب في ذعر مزدوج (double panic)، مما الذي يُعطّل البرنامج (program) فورًا وينهي أي تنظيف جارٍ. هذا جيد لبرنامج مثال، لكن ليس لا يُنصح به لكود إنتاجي (production).

### الإشارة للخيوط (Threads) للتوقف عن الاستماع للوظائف (Jobs)

مع جميع التغييرات التي أجريناها، يُترجم (compile) كودنا (code) بدون أي تحذيرات. ومع ذلك، الأخبار السيئة هي أن هذا الكود لا يعمل بالطريقة التي نريدها بعد. المفتاح هو المنطق في الإغلاقات (closures) التي يُشغّلها الخيوط (threads) من نسخ (instances) `Worker`: في الوقت الحالي، نستدعي `join`، لكن ذلك لن يُوقف الخيوط، لأنها تُكرّر `loop` للأبد بحثًا عن وظائف (jobs). إذا حاولنا إسقاط (drop) `ThreadPool` الخاص بنا مع تطبيقنا الحالي لـ `drop`، فسيُحجَب (block) الخيط الرئيسي (main thread) للأبد، منتظرًا للخيط (thread) الأول لينتهي.

لإصلاح هذه المشكلة، سنحتاج إلى تغيير في تطبيق `drop` لـ `ThreadPool` ثم تغيير في حلقة (loop) `Worker`.

أولاً، سنغيّر تطبيق `drop` لـ `ThreadPool` لإسقاط (drop) `sender` صراحةً قبل الانتظار للخيوط (threads) لتنتهي. تُظهر القائمة (Listing) 21-23 التغييرات على `ThreadPool` لإسقاط `sender` صراحةً. على عكس مع الخيط (thread)، هنا نحتاج فعلاً إلى استخدام `Option` لنتمكّن من نقل `sender` خارج `ThreadPool` مع `Option::take`.
...
- ابحث عن حزمة (crate) thread pool على [crates.io](https://crates.io/) ونفّذ خادوم ويب (web server) مشابهًا باستخدام الحزمة بدلاً. ثم، قارن واجهة (API) والمتانة الخاصة بها بمجمع الخيوط (thread pool) الذي نفّذناه.
