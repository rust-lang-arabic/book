...
الدوال المشروحة ويبلغ عما إذا كانت كل دالة اختبار تنجح (pass) أو
تفشل (fail).
...
التأكيد يعمل كمثال على صيغة الاختبار (test) النموذجي. لنقم بتشغيله
لنرى أن هذا الاختبار ينجح (passes).
...
passed; 0 failed` يجمع عدد الاختبارات التي نجحت أو فشلت (failed).
...
عندما يحدث panic في دالة الاختبار (test function). يتم تشغيل كل اختبار في خيط جديد
، وعندما يرى الخيط الرئيسي أن خيط اختبار (test thread) قد مات، يتم تحديد الاختبار
