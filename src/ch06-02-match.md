...
الطريقة، تمر القيم عبر كل نمط (pattern) في `match`، وعند أول نمط "تناسبه" القيمة، تسقط
...
<Listing number="6-3" caption="تعداد (enum) وتعبير `match` يحتوي على متغيرات (variants) التعداد كأنماطه (patterns)">
...
التالي هو أذرع (arms) `match`. يحتوي الذراع (arm) على جزأين: نمط (pattern) وبعض الكود. الذراع الأول
...
الكود المرتبط بكل ذراع (arm) هو تعبير، والقيمة الناتجة عن التعبير في الذراع المطابق
...
يمتلكها `u8`، لأن النمط (pattern) الأخير سيطابق جميع القيم غير المدرجة صراحة. هذا النمط
الذي يلتقط الكل يلبي المتطلب أن `match` يجب أن يكون شاملاً. لاحظ أنه يجب علينا
وضع ذراع (arm) التقاط الكل أخيراً لأن الأنماط (patterns) يتم تقييمها بالترتيب. إذا وضعنا ذراع
