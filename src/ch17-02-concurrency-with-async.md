...
في هذا القسم، سنطبق async على بعض تحديات التزامن (concurrency) نفسها التي تعاملنا معها باستخدام الخيوط (threads) في الفصل 16. نظرًا لأننا تحدثنا بالفعل عن الكثير من الأفكار الأساسية هناك، في هذا القسم سنركز على ما هو مختلف بين الخيوط والمستقبلات (futures).
