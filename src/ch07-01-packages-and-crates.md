...
--> في الفصل 1)، فإن المُصرِّف يعتبر ذلك الملف صندوقاً. يمكن أن تحتوي الصناديق (crates) على
وحدات (modules)، وقد تُعرَّف الوحدات في ملفات أخرى يتم تصريفها مع الصندوق، كما سنرى في
الأقسام القادمة.
...
في معظم الأوقات عندما يقول مستخدمو Rust «صندوق»، فإنهم يقصدون صندوق مكتبة (library crate)، ويستخدمون
...
بناء تلك الصناديق (crates). Cargo في الواقع حزمة تحتوي على الصندوق الثنائي (binary crate)
لأداة سطر الأوامر التي كنت تستخدمها لبناء كودك. حزمة Cargo
تحتوي أيضاً على صندوق مكتبة (library crate) يعتمد عليه الصندوق الثنائي. المشاريع الأخرى
...
مكتبة (library crate) واحد فقط. يجب أن تحتوي الحزمة على صندوق (crate) واحد على الأقل، سواء كان
صندوق مكتبة أو صندوق ثنائي (binary crate).
...
يحتوي على _src/lib.rs_، فإن الحزمة تحتوي على صندوق مكتبة (library crate)
...
صندوق ثنائي (binary crate) اسمه `my-project`. إذا كانت الحزمة تحتوي على _src/main.rs_
و _src/lib.rs_، فإنها تحتوي على صندوقين (crates): صندوق ثنائي وصندوق مكتبة (library crate)، كلاهما بنفس
اسم الحزمة. يمكن أن تحتوي الحزمة على صناديق ثنائية (binary crates) متعددة بوضع ملفات
