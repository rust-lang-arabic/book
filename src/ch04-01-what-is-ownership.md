...
بعض اللغات تحتوي على جامع قمامة (garbage collector) يبحث بانتظام عن الذاكرة التي لم تعد مستخدمة
أثناء تشغيل البرنامج؛ في لغات أخرى، يجب على المبرمج تخصيص (allocate) وتحرير (free) الذاكرة بشكل صريح.
تستخدم Rust نهجاً ثالثاً: تتم إدارة الذاكرة من خلال نظام ملكية (ownership system) مع مجموعة من القواعد
...
الميزات (features) التي تجعل Rust فريدة. في هذا الفصل، ستتعلم الملكية من خلال
...
> قيمة (value) على المكدس أو الكومة تؤثر على كيفية تصرف اللغة ولماذا
...
> استعداداً لذلك.
>
> كل من المكدس والكومة أجزاء من الذاكرة (memory) المتاحة لكودك لاستخدامها
> في وقت التشغيل (runtime)، لكنهما منظمان بطرق مختلفة. يخزن المكدس
...
> البيانات المخزنة على المكدس يجب أن يكون لها حجم معروف وثابت (known, fixed size). البيانات ذات الحجم
> غير المعروف في وقت الترجمة (compile time) أو الحجم الذي قد يتغير يجب أن تُخزن على الكومة
> بدلاً من ذلك.
>
> الكومة أقل تنظيماً: عندما تضع بيانات على الكومة، تطلب
> كمية معينة من المساحة. يجد مخصص الذاكرة (memory allocator) مكاناً فارغاً في الكومة
...
> المكدس (stack) لا يُعتبر تخصيصاً). نظراً لأن المؤشر إلى الكومة له
...
يحتوي على طول (5)، والسعة، ومؤشر إلى القيمة الأولى في
