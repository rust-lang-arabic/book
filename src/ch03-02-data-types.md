...
>
> لنفترض أن لديك متغيرًا (variable) من النوع (type) `u8` يمكنه الاحتفاظ بقيم (values) بين 0 و 255. إذا حاولت تغيير المتغير إلى قيمة (value) خارج هذا النطاق، مثل 256، فسيحدث _تجاوز العدد الصحيح_، والذي يمكن أن يؤدي إلى أحد سلوكين. عند الترجمة (compilation) في وضع التصحيح (debug mode)، يتضمن Rust فحوصات لتجاوز الأعداد الصحيحة التي تتسبب في _الذعر (panic)_ لبرنامجك في وقت التشغيل (runtime) إذا حدث هذا السلوك. يستخدم Rust مصطلح _الذعر_ عندما يخرج البرنامج مع خطأ؛ سنناقش حالات الذعر (panics) بشكل أكثر تفصيلاً في قسم [«الأخطاء غير القابلة للاسترداد مع `panic!`»][unrecoverable-errors-with-panic]<!-- ignore --> في الفصل التاسع.
>
> عند الترجمة في وضع الإصدار (release mode) باستخدام علامة `--release`، لا يتضمن Rust فحوصات لتجاوز الأعداد الصحيحة التي تسبب الذعر. بدلاً من ذلك، إذا حدث تجاوز (overflow)، يقوم Rust بإجراء _التفاف المتمم الثنائي (two's complement wrapping)_. باختصار، القيم الأكبر من القيمة القصوى التي يمكن للنوع الاحتفاظ بها «تلتف» إلى الحد الأدنى من القيم التي يمكن للنوع الاحتفاظ بها. في حالة `u8`، تصبح القيمة 256 هي 0، والقيمة 257 تصبح 1، وهكذا. لن يصاب البرنامج بالذعر، لكن المتغير سيكون له قيمة ربما لم تكن تتوقعها. يعتبر الاعتماد على سلوك التفاف تجاوز العدد الصحيح (integer overflow wrapping behavior) خطأً.
>
...
كما في معظم لغات البرمجة الأخرى، فإن النوع المنطقي (Boolean type) في Rust له قيمتان (values) ممكنتان: `true` و `false`. القيم المنطقية (Boolean values) بحجم بايت واحد. يتم تحديد النوع المنطقي في Rust باستخدام `bool`. على سبيل المثال:
...
يمكن للـ _أنواع المركبة (compound types)_ ترجمة قيم (values) متعددة في نوع (type) واحد. يحتوي Rust على نوعين مركبين أوليين: المجموعات (tuples) والمصفوفات (arrays).
...
نقوم بإنشاء مجموعة (tuple) عن طريق كتابة قائمة مفصولة بفواصل من القيم (values) داخل أقواس. كل موضع في المجموعة له نوع (type)، ولا يجب أن تكون أنواع (types) القيم المختلفة في المجموعة هي نفسها. لقد أضفنا تعليقات توضيحية (type annotations) اختيارية للنوع في هذا المثال:
...
يربط المتغير (variable) `tup` بالمجموعة (tuple) بأكملها لأن المجموعة تعتبر عنصرًا (element) مركبًا واحدًا. للحصول على القيم (values) الفردية من المجموعة، يمكننا استخدام مطابقة النمط (pattern matching) لتفكيك (destructuring) قيمة (value) المجموعة، مثل هذا:
...
ينشئ هذا البرنامج أولاً مجموعة (tuple) ويربطها بالمتغير (variable) `tup`. ثم يستخدم نمطًا (pattern) مع `let` لأخذ `tup` وتحويله إلى ثلاثة متغيرات (variables) منفصلة، `x` و `y` و `z`. يسمى هذا _التفكيك (destructuring)_ لأنه يكسر المجموعة الواحدة إلى ثلاثة أجزاء. أخيرًا، يطبع البرنامج قيمة (value) `y`، وهي `6.4`.
...
ينشئ هذا البرنامج المجموعة (tuple) `x` ثم يصل إلى كل عنصر (element) من عناصر (elements) المجموعة باستخدام فهارسها (indexes) الخاصة. كما هو الحال مع معظم لغات البرمجة، يكون الفهرس (index) الأول في المجموعة هو 0.

للمجموعة (tuple) بدون أي قيم (values) اسم خاص، _الوحدة (unit)_. يتم كتابة هذه القيمة (value) ونوعها (type) المقابل على حد سواء `()` وتمثل قيمة فارغة أو نوع إرجاع (return type) فارغ. تقوم التعبيرات (expressions) ضمنيًا بإرجاع قيمة الوحدة إذا لم ترجع أي قيمة أخرى.

#### نوع المصفوفة (Array Type)

طريقة أخرى لوجود مجموعة (collection) من قيم (values) متعددة هي باستخدام _مصفوفة (array)_. على عكس المجموعة (tuple)، يجب أن يكون لكل عنصر (element) من عناصر (elements) المصفوفة نفس النوع (type). على عكس المصفوفات (arrays) في بعض اللغات الأخرى، فإن المصفوفات في Rust لها طول ثابت.
...
تكون المصفوفات (arrays) مفيدة عندما تريد تخصيص بياناتك على المكدس (stack)، كما هو الحال مع الأنواع (types) الأخرى التي رأيناها حتى الآن، بدلاً من الكومة (heap) (سنناقش المكدس والكومة بمزيد من التفاصيل في [الفصل الرابع][stack-and-heap]<!-- ignore -->) أو عندما تريد التأكد من أن لديك دائمًا عددًا ثابتًا من العناصر (elements). المصفوفة (array) ليست مرنة مثل نوع المتجه (vector). المتجه هو نوع مجموعة (collection type) مماثل يوفره المكتبة القياسية (standard library) _يُسمح_ له بالنمو أو الانكماش في الحجم لأن محتوياته تعيش على الكومة. إذا لم تكن متأكدًا من استخدام مصفوفة أو متجه، فمن المحتمل أن تستخدم متجهًا. يناقش [الفصل الثامن][vectors]<!-- ignore --> المتجهات (vectors) بمزيد من التفاصيل.
...
تكتب نوع (type) المصفوفة (array) باستخدام أقواس مربعة مع نوع كل عنصر (element)، وفاصلة منقوطة، ثم عدد العناصر (elements) في المصفوفة، مثل هذا:
...
يمكنك أيضًا تهيئة مصفوفة (array) لتحتوي على نفس القيمة (value) لكل عنصر (element) عن طريق تحديد القيمة الأولية، متبوعة بفاصلة منقوطة، ثم طول المصفوفة بين أقواس مربعة، كما هو موضح هنا:
...
المصفوفة (array) هي جزء واحد من الذاكرة (memory) بحجم معروف وثابت يمكن تخصيصه على المكدس (stack). يمكنك الوصول إلى عناصر (elements) المصفوفة باستخدام الفهرسة (indexing)، مثل هذا:
...
في هذا المثال، سيحصل المتغير (variable) المسمى `first` على القيمة (value) `1` لأن هذه هي القيمة عند الفهرس (index) `[0]` في المصفوفة (array). سيحصل المتغير المسمى `second` على القيمة `2` من الفهرس `[1]` في المصفوفة.

#### الوصول غير الصالح إلى عنصر المصفوفة (Invalid Array Element Access)

دعنا نرى ما يحدث إذا حاولت الوصول إلى عنصر (element) من مصفوفة (array) يتجاوز نهاية المصفوفة. لنفترض أنك تقوم بتشغيل هذا الكود، على غرار لعبة التخمين في الفصل الثاني، للحصول على فهرس (index) مصفوفة من المستخدم:
...
يتم ترجمة (compilation) هذا الكود بنجاح. إذا قمت بتشغيل هذا الكود باستخدام `cargo run` وأدخلت `0` أو `1` أو `2` أو `3` أو `4`، فسيطبع البرنامج القيمة (value) المقابلة عند ذلك الفهرس (index) في المصفوفة (array). إذا أدخلت بدلاً من ذلك رقمًا يتجاوز نهاية المصفوفة، مثل `10`، فسترى إخراجًا مثل هذا:
...
أدى البرنامج إلى خطأ في وقت التشغيل (runtime) عند نقطة استخدام قيمة (value) غير صالحة في عملية الفهرسة (indexing). خرج البرنامج برسالة خطأ ولم ينفذ عبارة (statement) `println!` النهائية. عندما تحاول الوصول إلى عنصر (element) باستخدام الفهرسة، سيتحقق Rust من أن الفهرس (index) الذي حددته أقل من طول المصفوفة (array). إذا كان الفهرس أكبر من أو يساوي الطول، فسيصاب Rust بالذعر (panic). يجب أن يحدث هذا الفحص في وقت التشغيل، خاصة في هذه الحالة، لأن المصرِّف (compiler) لا يمكنه معرفة القيمة التي سيدخلها المستخدم عندما يقوم بتشغيل الكود لاحقًا.
