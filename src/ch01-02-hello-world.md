...
تعرّف هذه الأسطر دالة (function) تُسمى `main`. دالة `main` خاصة: إنها دائماً أول كود يتم تشغيله في كل برنامج Rust قابل للتنفيذ (executable). هنا، يعلن السطر الأول دالة تُسمى `main` لا تحتوي على معاملات (parameters) ولا تُرجع (return) شيئاً. إذا كانت هناك معاملات، فستكون داخل الأقواس (parentheses) (`()`).
